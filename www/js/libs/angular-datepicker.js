      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , resetToMinDate = function resetToMinDate() {
            $scope.month = getMes(new Date($scope.dateMinLimit).getMonth());// meses[new Date($scope.dateMinLimit).getMonth()];
          ...
          , resetToMaxDate = function resetToMaxDate() {
        	$scope.month = getMes(new Date($scope.dateMinLimit).getMonth());
//            $scope.month = $filter('date')(new Date($scope.dateMaxLimit), 'MMMM');
          ...
          , unregisterDataSetWatcher = $scope.$watch('dateSet', function dateSetWatcher(newValue) {
            ...
            if (newValue && !isNaN(Date.parse(newValue))) {
              ...
              $scope.month = getMes(new Date($scope.dateMinLimit).getMonth());//december-November like
        ...
        $scope.nextMonth = function nextMonth() {
          ...
          $scope.month = getMes(new Date($scope.year, $scope.monthNumber - 1).getMonth());
          //set next month
...
//          $scope.month = $filter('date')(new Date($scope.year, $scope.monthNumber - 1), 'MMMM');
        ...
        $scope.daysInString = $scope.daysInString.map(function mappingFunc(el) {
        	console.log(getDiaSemana(el))
        	console.log($filter('date')(new Date(new Date('06/08/2014').valueOf() + A_DAY_IN_MILLISECONDS * el), 'EEE'))
//          return $filter('date')(new Date(new Date('06/08/2014').valueOf() + A_DAY_IN_MILLISECONDS * el), 'EEE');
        	return $filter('date')(getDiaSemana(el));
        });
